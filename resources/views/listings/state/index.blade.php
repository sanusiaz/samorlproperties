...
@section('title', 'State - ' . ucfirst($state))


@section('products_meta_tags')
    <meta name="description" content="Verified Properties in  {{ ucfirst($state) }}" />
    <!-- Twitter Card data -->
    <meta name="twitter:card" content="Samorl Properties">
    <meta name="twitter:title" content="Properties in  {{ ucfirst($state) }}">
    <meta property="twitter:url" content="https://properties.samorl.com.ng/listings/state/{{ $state }}" />
    <meta name="twitter:description" content="Get Verified and featured property at fixed / negotiable price in {{ $state }}">
    <!-- Twitter Summary card images must be at least 120x120px -->
    <meta name="twitter:image" content="https://properties.samorl.com.ng/images/main_seo_image.jpg">
    
    
    <!-- Open Graph data -->
    <meta property="og:title" content="Properties in  {{ ucfirst($state) }}" />
    <meta property="og:type" content="article" />
    <meta property="og:url" content="https://properties.samorl.com.ng/listings/state/{{ $state }}" />
    <meta property="og:image" content="https://properties.samorl.com.ng/images/main_seo_image.jpg" />
    <meta property="og:description" content="Get Verified and featured property at fixed / negotiable price in {{ $state }}" />
...
@section('content')
	...
	<div class="flex flex-col items-center align-middle justify-center w-full h-52 text-center bg-cover bg-center short_banner relative"
		...
		<span class="font-bold uppercase text-white text-2xl md:text-4xl block w-full az_custom_front tracking-wide break-words z-50 pb-12 capitalize" >Properties In {{ $state }}</span>
	...
	<main class="p-0 m-0 mb-0 mt-10 mb-10 px-4 md:px-7 lg:px-20 relative overflow-hidden">
		...
		<div id="main_property" class="block relative pt-16 md:pt-4 mt-36 md:mt-5 px-0 px-0 sm:px-3 lg:px-8">
			@if ( $cities->count() > 0 )
			<!-- Cities Lists -->
			<div class="mb-5 px-5 pt-0 text-sm">
				@foreach( $cities as $city )
				<a href="{{ url('/listings/city/' . $state . '/' . strtolower($city->city_name)) }}" class="text-blue-700 rounded border border-blue-500 font-semibold w-max inline-block px-3 py-2 hover:bg-blue-500 hover:text-white"> {{ $city->city_name }} </a>
				@endforeach
			</div>
			@endif
			<!-- Property main view -->
